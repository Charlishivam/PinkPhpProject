 ...
 * @Email : adilkhanazad786@gmail.com
 * @version         1.1.7
 ...
 */
/*CI_Controller old */
class API_Controller extends MY_Api_Controller {
    /**
     ...
     */
    const HEADER_STATUS_STRINGS = ['405' => 'HTTP/1.1 405 Method Not Allowed', '400' => 'BAD REQUEST', '408' => 'Request Timeout', '404' => 'NOT FOUND', '401' => 'UNAUTHORIZED', '200' => 'OK', ];
    /**
        ...
        if (strtolower($api_key_type) == 'header') {
            ...
            if (isset($is_header['status']) === TRUE) {
                ...
                if ($api_key != "table") {
                    ...
                    $where_key_data = ['controller' => $this->router->fetch_class(), 'api_key' => $HEADER_VALUE, ];
        ...
        } else if (strtolower($api_key_type) == 'get') // // api key type `get`
        {
            ...
            if (!empty($get_param_value) AND is_string($get_param_value)) {
                ...
                if ($api_key != "table") {
                    ...
                    $where_key_data = ['controller' => $this->router->fetch_class(), 'api_key' => $get_param_value, ];
                    $limit_query = $this->db->get_where($this->API_KEYS_TABLE_NAME, $where_key_data);
                    if (!$this->db->affected_rows() > 0) {
                        $this->_response(['status' => FALSE, 'error' => 'API Key Invalid'], self::HTTP_NOT_FOUND);
                    }
